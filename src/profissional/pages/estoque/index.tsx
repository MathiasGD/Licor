...
const colunas: GridColDef[] = [
  ...
  },
  {
    ...
    width: 120,
    flex: 1,
    valueGetter: (value, row) => row.estoque?.dataAtualizado.toLocaleDateString()
  }
]
...
function Estoque() {
  ...
  const [registroIngredienteId, setRegistroIngredienteId] = useState<number | null>(null);
  const [registroQuantidade, setRegistroQuantidade] = useState<number | null>(null);
  
  return (
    <div className="flex flex-col gap-4 w-full items-start">
      <div className="flex gap-2">
        <Button variant="contained" disableElevation onClick={() => setOpenModalCadastro(true)}>Cadastrar ingrediente</Button>
        <Button variant="contained" disableElevation onClick={() => setOpenModalRegistro(true)}>Registrar em estoque</Button>
      </div>
      ...
      <Modal isOpen={openModalCadastro}>
        <div className="flex flex-col justify-between gap-3 min-h-full">
          <div className="flex flex-col gap-3">
            <p className="text-xl font-medium text-roxo-escuro self-center">Novo ingrediente</p>
            <TextField fullWidth placeholder="Nome" value={nomeNovoIngrediente} onChange={(e) => setNomeNovoIngrediente(e.target.value)} />
            <div className="w-full h-px bg-branco-muito-escuro" />
            <TextField fullWidth placeholder="Descrição" value={descricaoNovoIngrediente} onChange={(e) => setDescricaoNovoIngrediente(e.target.value)} />
          </div>
      ...
      <Modal isOpen={openModalRegistro}>
        <div className="flex flex-col justify-between gap-3 min-h-full">
          <div className="flex flex-col gap-3">
            <p className="text-xl font-medium text-roxo-escuro self-center">Registrar em estoque</p>
            <div className="flex flex-col gap-1">
                <p className="text-sm">Ingrediente:</p>
                <Select size="small" fullWidth value={registroIngredienteId} onChange={(e) => setRegistroIngredienteId(Number(e.target.value))}>
            ...
            <div className="w-full h-px bg-branco-muito-escuro" />
            <TextField fullWidth placeholder="Quantidade" value={registroQuantidade} type="number" onChange={(e) => setRegistroQuantidade(Number(e.target.value))} />
          </div>
          <div className="flex flex-col gap-2 w-full">
