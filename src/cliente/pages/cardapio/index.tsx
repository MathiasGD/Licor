...
import { useCadastrarPedido, useDrinks, type Drink } from "../../../services/geral";
    ...
    <div className="flex flex-col items-center">
      <Modal isOpen={!cliente}>
        <div className="flex flex-col items-center gap-4">
          <p className="text-roxo-escuro text-xl font-medium">Como você quer ser identificado?</p>
          <TextField fullWidth placeholder="Identificador" value={preCliente} onChange={(e) => setPreCliente(e.target.value)} />
          <Button fullWidth variant="contained" onClick={() => setCliente(preCliente)} disableElevation>Confirmar</Button>
        </div>
      </Modal>
          ...
          <div className="flex flex-col items-center justify-between h-full">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="flex flex-col items-center gap-2 text-sm">
                ...
                {drinkSelecionado.composicao.map((ingrediente) => <p key={ingrediente.id} className="text-preto-fraco"> {ingrediente.ingrediente.nome}</p>)}
              </div>
              <div className="w-full h-px bg-branco-muito-escuro" />
            </div>
            <div className="flex flex-col gap-2 w-full">
              ...
              <Button variant="contained" onClick={() => {
                cadastrarPedido.mutateAsync({ cliente, drinkId: drinkSelecionado.id })
                setDrinkSelecionado(null);
              }} disableElevation>Fazer pedido | {drinkSelecionado.precoBase}$</Button>
            </div>
          </div>
        )}     
      </Modal>
